...
func (tc *testChain) GenRandomTree(genBlocks int) {
	for i := 0; i < genBlocks; i++ {
		...
		// Add randomness to header to create slightly different blocks
		h := block.Header()
		randData := make([]byte, 20)
		if _, err := rand.Read(randData); err != nil {
			tc.t.Fatal(err)
		}
		h.Coinbase = common.BytesToAddress(randData)
...
func TestChain(t *testing.T) {
	...
	chainConfig.FeeConfig.MaxBlockGasCost = common.Big0
