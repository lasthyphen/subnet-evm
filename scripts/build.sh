...
if [[ $# -eq 1 ]]; then
    ...
    binary_path="$GOPATH/src/github.com/lasthyphen/dijetsnodego/build/plugins/srEXiWaHuhNyGwPUi444Tu47ZEDwxTWrbQiuD7FmgSAQ6X7Dy"
else
...
go build -ldflags "-X github.com/lasthyphen/subnet-evm/plugin/evm.GitCommit=$subnet_evm_commit -X github.com/lasthyphen/subnet-evm/plugin/evm.Version=$subnet_evm_version $static_ld_flags" -o "$binary_path" "plugin/"*.go
